...
public class CreaRegistro_end_baja {

	public static final String plantilla = CtsArmado.pathJar + "Plantilla GM07 Baja Color.pdf";
	...
	public CreaRegistro_end_baja(String nNegocio) {
	...
	public static void main(String[] args) {
		try{
			CreaRegistro_end_baja cc = new CreaRegistro_end_baja("");		
		...
		br = new BufferedReader(new FileReader(f));    
			...
			while(((line = br.readLine()) != null)&&(cont<48)){
				if(!line.trim().equals("DJD1 FORM=NULA,END;")){
					if(line.length()>0){
						if((line.trim().equals("DJD1 FEED='SEGU1',PMODE=POR,FORM=AZ48,JDE=AZ48,DUP=YES,END;"))||(line.trim().equals("DJD1 FEED='SEGU1',JDE=GM07,PMODE=POR,DUP=NO,FORM=GM07,END;"))||(line.trim().equals("DJD1 FEED='SEGU1',FORM=GM07A,END;"))){
							datos[cont]="";
						}else{
							datos[cont]=line.substring(1, line.length());
						}
...
//					boolean ultimaPag = false;
	...
	public void creaCaraCte(String[] datos) throws IOException, DocumentException{
		...
		ColumnText.showTextAligned(canvas, Element.ALIGN_LEFT, phraset, 20, 719, 0);         
		System.out.println("Generando c0: " + phraset);	
		
		Arial.setSize((float) 10);
		Arial.setStyle(1);
		phraset = new Phrase(c01);        
		ColumnText.showTextAligned(canvas, Element.ALIGN_LEFT, phraset, 530, 719, 0);         
		System.out.println("Generando c01: " + phraset);
		
		Arial.setSize((float) 10);
		Arial.setStyle(0);
//		phraset = new Phrase(c100);        
//		ColumnText.showTextAligned(canvas, Element.ALIGN_LEFT, phraset, 40, 540, 0);         p
